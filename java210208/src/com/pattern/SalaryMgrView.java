 ...
 * static을 사용하는 것은 반칙이다.(학습목표인 생성자 공부가 안된다.)
 * 뷰계층-업무처리계층-이벤트 처리 핸들러 클래스 3개로 나눠본다.
 * 
 * 누가 누구를 인스턴스화 해야 하는가?
 * :A안에 B를 인스턴스화 해야 하나? 아니면 B안에 A를 인스턴스화 해야 하나요?
 * 생성자의 파라미터 자리에는 무엇을 써야 할까요? - 생성자 오버로딩에 대해서는 알고있는거니?
 * -SalaryMgrView (이것을 가리키는 것은 this)
 * -SalaryMgrHandler
 * -SalaryMgrLogic
 * 한개만, 두개가, 세개가 다 필요한지(원하는지) 생각해보기
 * 세 개의 클래스를 그림으로 표시 한 뒤 화살표를 통해서  객체 주입관계를 완성하시오.
 */
